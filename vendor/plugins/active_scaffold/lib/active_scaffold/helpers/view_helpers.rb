  ...
  module Helpers
    ...
    module ViewHelpers
      ...
      def render_action_link(link, url_options)
        ...
        if (link.crud_type != :read) or (label == "Mostrar")
          if link.crud_type == :create
            html_options.merge!(:style => "width: 16px;height: 16px")
            link_to "", url_options, html_options
          elsif label == "Modificar" or label == "Borrar" or label == "Mostrar"
            link_to image_tag("#{link.crud_type}.png" , :size => "16x16", :alt =>
                label), url_options, html_options
          else
